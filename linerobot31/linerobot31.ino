...
#include<Servo.h>
Servo servo;
const uint8_t        pinSensorL   = A0;                             // Вывод к которому подключен датчик находящийся слева         (по направлению движения)
const uint8_t        pinSensorC   = A2;                             // Вывод к которому подключен датчик находящийся по центру     (по направлению движения)
const uint8_t        pinSensorR   = A1;                             // Вывод к которому подключен датчик находящийся справа        (по направлению движения)
const uint8_t        pinShield_LH = 8;                              // Вывод направления к которому подключен левый мотор          (по направлению движения)
const uint8_t        pinShield_LE = 11;                              // Вывод ШИМ         к которому подключен левый мотор          (по направлению движения)
//                   ОБЪЕКТЫ:

//                   УСТАНАВЛИВАЕМЫЕ ЗНАЧЕНИЯ:
const uint16_t       valSensor1   = 730;                            // Показание датчика находящегося на линии                     (указывается для конкретной трассы)
const uint16_t       valSensor0   = 32;                            // Показание датчика находящегося вне линии                    (указывается для конкретной трассы)
const uint8_t        valSpeed     = 255;                            // Максимальная скорость                                       (число от 1 до 255)
const uint32_t       tmrDelay     = 5000;                           // Время в течении которого требуется остановиться             (если в течении этого времени состояние остаётся неопределённым (линия не обнаружена), то требуется остановиться)
...
//                   РАССЧИТЫВАЕМЫЕ ЗНАЧЕНИЯ:
      uint8_t        arrSpeed;                                   // Рассчитываемая скорость для каждого мотора                  (число от 1 до valSpeed, нулевой элемент - правый мотор, первый элемент - левый мотор)
      ...
      bool           flgservo;  
      bool           flgDistance;// Флаг указывающий на то, что требуется изменить ШИМ моторов  (0 - тёмная линия, 1 - светлая линия)
...
//  Устанавливаем флаг ШИМ, сбрасываем флаг наличия поворота, флаг остановки и флаг обнаружения припятствий
    flgservo=1; flgTurning=0; flgStop=0; flgDistance=0;
//  Устанавливаем режим работы выводов и направление обоих моторов
    ...
    digitalWrite(pinShield_LH, 1);                                  // Устанавливаем на выходе pinShield_LH уровень arrRoute[1]    (направление движения левого мотора)
...
//  Определяем действия в соответствии с текущим положением датчиков
    switch(valSensor){ //            Сохраняем время:    Меняем флаг ШИМ:   Меняем флаг поворота:  Меняем флаг остановки:
        case 0b000:                                      flgservo=flgTurning;                        flgStop=!flgTurning;Serial.println("case000");  break;  // 000 - Если все датчики находятся вне линии                 (неопределённое состояние - если до этого мы не поворачивали, то резко останавливаемся, а если поворачивали, то продолжаем поворот в ту же сторону)
        case 0b010:                 tmrMillis=millis(); flgservo=flgTurning; flgTurning=0;          flgStop=0;     Serial.println("forward010");     break;  // 010 - Если только центральный датчик находится на линии    (продолжаем движение прямо)
        case 0b100:                  tmrMillis=millis(); flgservo=1;          flgTurning=-1;         flgStop=0;     Serial.println("left100");         break;                                                                                           // 100 - Если только левый датчик находится на линии          (поворачиваем влево)
        case 0b110:                  tmrMillis=millis(); flgservo=1;          flgTurning=-1;         flgStop=0;     Serial.println("left110");       break;  // 110 - Если левый и центральный датчики находятся на линии  (поворачиваем влево)
        case 0b001:                  tmrMillis=millis(); flgservo=1;          flgTurning=1;          flgStop=0;     Serial.println("right001");      break;                                                                                           // 001 - Если только правый датчик находится на линии         (поворачиваем вправо)
        case 0b011:                  tmrMillis=millis(); flgservo=1;          flgTurning=1;          flgStop=0;     Serial.println("right011");      break;  // 011 - Если правый и центральный датчики находятся на линии (поворачиваем вправо)
        default:                                         flgservo=1;                                                Serial.println("case111");       break;  // 1x1 - Если правый и левый датчики находятся на линии       (неопределённое состояние)
    }
    if(tmrMillis>millis())         { tmrMillis=0;}                                                                              // Избавляемся от переполнения millis();
    if(tmrMillis+tmrDelay<millis()){                     flgservo=1;          flgTurning=0;          flgStop=1;           }       // Останавливаемся если линия потеряна на более чем tmrDelay мс
    
//  Устанавливаем ШИМ для моторов
    if(flgservo){flgservo=0;                                                                                                        // Если установлен флаг flgPWM, то сбрасываем его и устанавливаем ШИМ ...
      switch(flgTurning){ // Скорость левого мотора:           Скорость правого мотора:
        case -1:             arrSpeed=175;                     servo.write(60);                                 break;  // Уменьшаем скорость левого  мотора (поворачиваем налево)
        case  0:             arrSpeed=valSpeed;                servo.write(100);                                break;  // Устанавливаем одинаковую скорость (едим прямо)
        case  1:             arrSpeed=175;                     servo.write(140);                                 break;  // Уменьшаем скорость правого мотора (поворачиваем направо)
      } if(flgStop){         arrSpeed=0;                   }                                                  // Останавливаемся если установлен флаг flgStop
       
//      Выводим ШИМ
        analogWrite(pinShield_LE, arrSpeed);
