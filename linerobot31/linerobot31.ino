...
//  Определяем действия в соответствии с текущим положением датчиков
    switch(valSensor){ //            Сохраняем время:    Меняем флаг ШИМ:   Меняем флаг поворота:  Меняем флаг остановки:
        case 0b000:                                      flgPWM=flgTurning;                        flgStop=!flgTurning;Serial.println("case000");break;  // 000 - Если все датчики находятся вне линии                 (неопределённое состояние - если до этого мы не поворачивали, то резко останавливаемся, а если поворачивали, то продолжаем поворот в ту же сторону)
        case 0b010:                 tmrMillis=millis(); flgPWM=flgTurning; flgTurning=0;          flgStop=0;     Serial.println("forward010");         break;  // 010 - Если только центральный датчик находится на линии    (продолжаем движение прямо)
        ...
        case 0b110:                  tmrMillis=millis(); flgPWM=1;          flgTurning=-1;         flgStop=0;     Serial.println("left110");       break;  // 110 - Если левый и центральный датчики находятся на линии  (поворачиваем влево)
        case 0b001:                  tmrMillis=millis(); flgPWM=1;          flgTurning=1;          flgStop=0;     Serial.println("right001");       break;                                                                                           // 001 - Если только правый датчик находится на линии         (поворачиваем вправо)
        case 0b011:                  tmrMillis=millis(); flgPWM=1;          flgTurning=1;          flgStop=0;     Serial.println("right011");       break;  // 011 - Если правый и центральный датчики находятся на линии (поворачиваем вправо)
        default:                                         flgPWM=1;                                                Serial.println("case111");      break;  // 1x1 - Если правый и левый датчики находятся на линии       (неопределённое состояние)
    }
